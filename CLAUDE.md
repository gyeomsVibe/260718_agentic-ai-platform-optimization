...
### SAFE-SYNC 게이트 (매 동기화 전 반드시 순서대로)

원치 않는 파일의 외부 유출을 막기 위한 안전 로직. 핵심 원칙:
**`git add -A`·`git add .`로 싹 담지 않는다. 내가 이번 세션에 만든 변경만 명시적 경로로 담는다.**

1. **열거**: `git status --porcelain`와 `git diff --stat`로 모든 미커밋 변경·미푸시 커밋을 나열.
2. **출처 분류**: 각 변경을 ⓐ 내가 이번 세션에 직접 만든 것 / ⓑ 내가 안 만든 것(다른
   세션·린터·사용자 직접 편집·정체불명)으로 나눈다.
3. **자동 대상은 ⓐ만**: ⓐ를 파일 경로로 **명시 스테이징**(`git add <경로>`).
4. **ⓑ는 자동 커밋 금지**: 내가 안 만든 변경은 자동으로 올리지 않고 **사용자에게 보고**한 뒤
   지시를 받는다. (단, 내 작업과 명백히 한 덩어리인 사소한 변경은 판단 하에 포함하고 보고에 명시)
5. **비밀 스캔**: 스테이징된 diff에서 토큰·키·비밀번호·쿠키·`.env` 패턴을 스캔. 하나라도
   걸리면 **중단하고 보고**한다(자동 진행 금지).
6. **대용량·바이너리 게이트**: 스테이징 후보에 바이너리/이미지/1MB 초과 파일이 있으면
   소스인지 확인. 소스가 아니면 `.gitignore`에 넣고 제외한다. 무시 파일을 `-f`로 강제 추가하지 않는다.
7. **커밋**: 의미 단위로 나눠 Conventional Commits로 작성.
8. **푸시**: 원격이 앞서면 `git pull --rebase` 후 push. `--force`·히스토리 재작성 금지.
9. **불확실하면 멈춘다**: 위 어느 단계든 판단이 애매하면 자동 진행 대신 사용자에게 확인.

- 커밋 안 된 변경이나 미푸시 커밋을 세션 끝에 남기지 않는다. 단, 게이트 4·5·9에 걸려
  **보류한 항목**은 "왜 보류했는지"와 함께 보고하는 것으로 갈음한다(억지로 올리지 않는다).
