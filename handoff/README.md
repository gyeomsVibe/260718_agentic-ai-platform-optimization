...
일반 작업은 검증된 작업 커밋 1개와 일반 push로 끝납니다. handoff 레코드와 별도 인계 커밋은
윤겸스가 `인계`, `handoff`, 플랫폼 간 재개를 명시적으로 요청했을 때만 만듭니다. 작업 종료,
파일 저장, 작은 유지보수, 에이전트 교체만으로는 인계를 만들지 않습니다.
...
인계가 명시적으로 요청된 경우에도 커밋 하나만으로 인계가 끝나지 않습니다. 다음 조건을 모두
만족할 때만 `HANDOFF_READY`입니다:
