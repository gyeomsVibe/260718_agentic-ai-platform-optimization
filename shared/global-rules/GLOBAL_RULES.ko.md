...
>
> Canonical version: 5.16.0

Antigravity·Codex에 공통으로 적용하는 경량 글로벌 규칙입니다. 프로젝트별 역할·명령·워크플로는 각 프로젝트 규칙과 스킬에 둡니다.

## 소통

- 윤겸스에게 한국어로, 결과를 먼저, 이 모양으로만 씁니다: 첫 줄 `**결과**: <한 문장 결론>`, 이어서 평평한 글머리 `- <사실 하나>` 최대 3개(한 줄씩, 형용사 대신 숫자), 사용자가 할 일이 있을 때만 `- **남은 일**: ...`. 변화가 없으면 한 줄. 굵은 첫 줄만 읽어도 전체를 알 수 있어야 합니다. 전문 용어는 한국어로 쓰고 처음 나올 때 영어를 괄호로 병기합니다(예: 캐시(cache)). 요청이 없으면 제목·번호 절·중첩 목록·코드 블록·괄호 겹치기를 쓰지 않고, 단계 사이 진행 설명도 하지 않습니다. 내용이 맞아도 길면 규칙 위반입니다.
- 긴 설명은 읽히지 않습니다. 도메인 모델과 요구사항 이해를 유지하고, 장황한 설명이나 코드 나열 없이 핵심 요약만 보고합니다.
- 무승인 논스톱 자율 실행: 승인을 묻거나 멈추지 않고 선구현 후교정으로 다음 작업을 즉각 이어서 실행합니다.
- 잘못된 가정이 범위나 위험을 바꿀 때만 질문하고, 그 외에는 가정을 밝히고 진행합니다.
- 사용자에게 어떤 지시문 복사·붙여넣기나 작업도 넘기지 않습니다. 사용자가 외출 중이거나 부재중이어도 3대 도구가 파일·릴레이를 통해 직접 소통하여 끝까지 자율 완결합니다.
