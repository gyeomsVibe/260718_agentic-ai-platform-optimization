...
| 도구 | 글로벌 파일 | 역할 |
|---|---|---|
| Codex | `~/.codex/AGENTS.md` | 지휘. 계획·순서·게이트·최종 판정을 소유하고 위임은 CLI/SQLite pilot 한 경로로만. 판정은 요약·diff·테스트로만 하며 빈 출력은 실패. 위임·판정 프롬프트의 고정부는 바이트 단위로 같게 두고 가변부는 뒤에 붙여 캐시를 안정화. 프로젝트에 로컬 작업자가 있으면 과제마다 고른다: 줄 단위로 적을 수 있고 파일이 적고 합격 기준이 기계적이면 로컬, 설계 판단·탐색·다파일 리팩터면 원격. 원격이 한도에 걸리면 같은 과제를 로컬로 1회 재시도하고 기록한다 |
