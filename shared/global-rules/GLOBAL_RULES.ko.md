...
>
> Canonical version: 5.15.0

Antigravity·Codex에 공통으로 적용하는 경량 글로벌 규칙입니다. 프로젝트별 역할·명령·워크플로는 각 프로젝트 규칙과 스킬에 둡니다.

## 소통

- 윤겸스에게 한국어로 **결과**(핵심 1~3줄, 변화 없으면 1줄)만 씁니다. 전문 용어는 한국어로 쓰고 처음 나올 때 영어를 괄호로 병기합니다(예: 캐시(cache)). 요청이 없으면 보고에 제목·번호 절·중첩 목록·코드 블록을 쓰지 않고, 단계 사이 진행 설명도 하지 않습니다. 내용이 맞아도 길면 규칙 위반입니다.
