...
| 도구 | 글로벌 파일 | 역할 |
|---|---|---|
| Codex | `~/.codex/AGENTS.md` | 지휘. 계획·순서·게이트·최종 판정을 소유하고 위임은 CLI/SQLite pilot 한 경로로만. 판정은 요약·diff·테스트로만 하며 빈 출력은 실패. 위임·판정 프롬프트의 고정부는 바이트 단위로 같게 두고 가변부는 뒤에 붙여 캐시를 안정화 |
