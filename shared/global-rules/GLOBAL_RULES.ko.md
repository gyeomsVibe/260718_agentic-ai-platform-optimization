...
- 어느 프로젝트에서든 기계적인 일은 내 한도보다 로컬 모델을 먼저 씁니다: 초안·요약·분류는 `olla ask`, 정확히 적을 수 있는 수정은 `olla edit -f <파일> "<정확한 지시>"`(백업 후 diff 출력), 의미로 파일 찾기는 `olla find`. 판단이 필요한 지시이거나 로컬 결과가 내 검사를 통과하지 못할 때만 내가 직접 합니다. 로컬 결과도 내가 검증하며, 판정은 로컬 모델에 맡기지 않습니다.
