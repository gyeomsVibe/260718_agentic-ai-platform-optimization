...
3. **금지선**: 비밀정보, 머신 설정, 대용량 파일, 이력 재작성은 기본 차단합니다.
4. **판정 가능성**: 자동화가 예 또는 아니오로 판단할 수 있는 조건만 규칙으로 만듭니다.
5. **최소 권한**: 전체 작업 트리가 아니라 승인된 저장소와 명시 경로만 다룹니다.
6. **검증 선행**: 저장소가 정한 테스트나 문서 검사를 통과한 뒤 커밋합니다.
7. **불확실성 정지**: 출처, 민감성, 원격 상태가 모호하면 추측하지 않고 보류합니다.
8. **가역성**: force push나 자동 rebase 대신 새 커밋과 명시적 복구 절차를 사용합니다.
9. **관찰 가능성**: 실행 결과에 대상 파일, 검사 결과, 커밋, 원격 상태, 보류 사유를 남깁니다.
10. **순복잡도 제한**: 새 규칙의 안전 효과가 유지 비용보다 클 때만 공통 규칙에 올립니다.
...
```mermaid
flowchart TD
  A["실제 변경 완료 또는 명시적 동기화 요청"] --> B["기준 상태와 소유 경로 기록"]
  B --> C["변경 출처 분류"]
  C --> D{"소유권이 명확한가?"}
  D -- "아니오" --> H["보류하고 이유 보고"]
