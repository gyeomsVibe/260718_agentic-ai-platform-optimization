...
| 외부 브리지 `mcp-server-google-antigravity` | ⏸ **미검증 — 승인 대기** | Windows 지원 명시(8.3 단축경로 처리 포함)·MIT. 단 **star 2개 수준의 저성숙도**. 실험 1 F4 이후 **우선순위 상승** — 샌드박스를 유지한 채 위임을 성립시킬 유일한 후보 | §8 R4 |
...
■ S6.4  실행자가 샌드박스 밖 자격증명을 요구하면, 샌드박스 해제 대신 MCP 경유를 먼저 검토한다.
        해제는 게이트 G2 승인 대상이며, 승인받아도 실행 환경의 별도 통제가 남을 수 있다. (F4·F5)
```
...
| R4 | 🔺 **외부 MCP 브리지 설치** (`mcp-server-google-antigravity` 등) | star 2~6개 수준 저성숙 패키지가 파일시스템 도구를 노출. 공급망 위험 | **우선순위 상향.** 실험 1 F4로 셸 호출 경로가 막히면서, 이제 **샌드박스를 유지한 채 위임을 성립시킬 유일한 후보**가 됐다. 다만 위험 자체는 그대로다. 설치 전 소스·의존성 감사를 선행할 것 |
