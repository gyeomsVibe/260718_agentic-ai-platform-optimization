...
> ### ✅ 2026-08-21 최종 개정 — 3층 구조는 성립한다. 단 전송 방식이 권한 경계를 결정한다
>
> 실험 1~5를 거치며 판정이 두 번 뒤집혔다. **최종 상태는 다음과 같다.**
>
> 1. **1층↔2층 (Codex → Antigravity)**: ✅ **성립한다 — 샌드박스를 켠 채로, 자동승인 없이.**
>    단 **셸 호출이 아니라 MCP 경유**여야 한다. 셸로 부르면 완화책 5종을 다 써도 `agy` 인증이 실패하지만(F4),
>    MCP 서버로 부르면 통과한다(F18). MCP 서버 프로세스가 Codex의 명령 샌드박스 밖에서 실행되기 때문이다.
>    실측: `AUTH_SUCCEEDED: yes`, `auto_approve: false`, `sandbox: true`.
> 2. **3층 (Claude Code)**: ❌ **Agent Teams는 이 버전에서 성립하지 않는다.** `SendMessage`는 있으나 `ListAgents`가
>    없고 `Agent` 도구에 `name` 파라미터가 없어, 띄운 에이전트들이 서로를 발견하지 못한다(F9).
>    성립하는 것은 팀 메시(mesh)가 아니라 **허브-앤-스포크**다. v2.1.237 업그레이드 후 재검증 대상.
> 3. **교차 검증**: ✅ **작동한다.** 감독자가 하우스키핑 14개가 실제로 생성된 상태에서
>    프로젝트 작업트리 무변화를 정확히 판정하고 오보를 내지 않았다(F22).
>
> #### 중간에 뒤집혔던 판정 (기록용)
>
> 실험 1·2 시점에는 *"샌드박스 해제가 이 기법의 필수 전제"* 라고 적었다. **이는 틀렸다.**
> 자료 C가 `전체 액세스`를 켠 것은 사실이지만 그것이 유일한 길은 아니었다.
> 셸 경로를 고집할 때만 치르는 대가였다. §8 R1은 그래서 두 번 조정됐다.
>
...
| **Codex → agy 셸 호출** (샌드박스 O) | 🧪 ❌ **불가** | 실험 1: read-only·workspace-write·전면읽기·네트워크허용·환경변수전체상속 **5가지 완화책 모두 실패**. 자격증명 접근 차단 | ↓ MCP 경유 |
| **Codex → agy 셸 호출** (샌드박스 해제) | ⏸ 미검증 | 로컬 classifier가 명령을 차단해 미실행. **이제 시도할 이유가 없다** — MCP 경유가 샌드박스를 켠 채 되기 때문 | 불필요 |
| **Codex → agy MCP 경유** (샌드박스 O) | 🧪 ✅ **가능 — 실행 확인** | 실험 4: `AUTH_SUCCEEDED: yes`, `auto_approve: false`, `sandbox: true`, 작업 `done`. 43초. 무인 실행은 `codex exec --approve-for-me` (`-s` 병용 불가) | — |
...
■ S6.6  무인 MCP 위임은 codex exec --approve-for-me 로 실행한다.
        exec 의 기본 승인 정책은 never 라 MCP 도구 호출이 막히며,
        --approve-for-me 는 -s 와 병용할 수 없다(자체 샌드박스를 적용한다).           (F19)
```

#### v2.3 정정 — S6.4 재개정 (2026-08-21, 실험 4)

```
■ S6.4 (최종)  실행자가 샌드박스 밖 자격증명을 요구하면 MCP 경유를 먼저 시도한다.
        MCP 서버 프로세스는 오케스트레이터의 명령 샌드박스 밖에서 실행되므로,
        셸 호출이 실패하는 자격증명 접근이 MCP 경유로는 성립한다.       (F18)
        단 브리지의 자동승인 기본값을 반드시 확인한다. 상류 그대로면 브리지가
        내부에서 skip-permissions 를 붙일 수 있다. 필요하면 하드닝 후 쓴다. (F17)
        샌드박스 해제는 최후 수단이며, MCP 경유가 가능하면 채택하지 않는다.

  ※ 이 조항은 하루 사이 두 번 바뀌었다.
     v2.1: "샌드박스 해제 대신 MCP 경유를 먼저 검토한다"        (실험 1 F4)
     v2.2: "MCP 경유가 자동으로 더 안전하지 않다"                (감사 F17)
     v2.3: "MCP 경유를 먼저 시도하되 자동승인 기본값을 확인한다"  (실험 4 F18)
     둘 다 참이었다. F17은 브리지의 기본값에 관한 것이고, F18은 전송 방식에 관한 것이다.
```

#### v2.3 추가 — S5.5

```
■ S5.5  교차 검증 결과는 감독자 보고를 그대로 채택하지 않는다.
        최소 1개 지표(작업트리 줄 수·실행자 하우스키핑 증가분)를 독립 확인한다.   (F22)
```
...
| 대상 | 판단 | 근거 |
|---|---|---|
| **`agy` 직접 헤드리스 위임** | 🔁 **Iterate** | 작동한다. 다만 S3.4(세 신호 분리)·S3.5(플래그 순서)를 지키지 않으면 조용히 오작동한다 |
| **Codex → MCP 브리지 → Antigravity** | 📈 **Scale (2026-08-21 상향)** | 실험 4·5에서 샌드박스 유지 + 자동승인 없이 성립. 3층 구조의 1층↔2층이 실제로 연결됐다. 주문: `codex exec --approve-for-me` |
| **Codex 경유 셸 호출** | ⏹ **Stop** | MCP 경유가 되는 이상 샌드박스를 꺼가며 셸로 부를 이유가 없다 |
...
### 추가로 수행한 것 (2026-08-21 후반)

- **소스 감사 → 최소 포크 벤더링 → 설치 → 등록 → 런타임 검증**까지 전 과정 완료.
- **실험 4·5** 수행. 실험 2에서 막혔던 측정을 위험 플래그 없이 완료했다.

### 여전히 검증하지 못한 것

- **진짜 위반(true positive)에 대한 적발 능력** — 측정한 것은 감독자의 **판별력**(하우스키핑을 위반으로 오보하지 않는 능력)이다. 실행자가 2회 시행 모두 제약을 지켜 **실제 위반이 한 번도 발생하지 않았고**, 따라서 잡아내는 능력은 여전히 미측정이다. 자료 C가 보고한 위반 사례는 재현되지 않았다.
