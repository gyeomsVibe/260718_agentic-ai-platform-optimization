# Codex App Pet Mod ([참조](https://github.com/openai/codex/issues/20863))
...
`assets/oVRedOdhq0.mp4` *(이 역사 자료에는 포함되지 않음)*

The demo uses the bundled Datachan Extended pet in `assets/datachan-extended` *(not included in this historical snapshot)*. It is a normal Codex custom pet folder plus an `animation` config that exercises longer row playback, custom timing, idle/running chains, and a hover override.

## What This Mod Does

The original draft expected Advanced Pet System TypeScript source code at `src/advanced-pet-system.ts`, but that source is not included in this historical snapshot. The proposed patch script would adapt the compiled system into the Codex desktop bundle.
...
The draft referred to `docs/configuration.md`, which is not included in this historical snapshot.

The draft also referred to `docs/upstream-issue.md`, which is not included in this historical snapshot.
