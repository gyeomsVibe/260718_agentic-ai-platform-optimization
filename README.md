...
### 2026-09-08 — 모델·추론 강도 사전 배차 글로벌 룰

| 변경 | 내용 |
|---|---|
| **공통 원칙** | 새 작업 경계에서 위험·범위·검증 가능성을 판정하고 품질 하한을 만족하는 가장 경제적인 모델·추론 강도를 선택 |
| **안전 경계** | 고위험 작업의 비용 강등 금지, 한 등급·한 번 승격, 요청 모델과 실제 모델 불일치 시 실패 처리 |
| **플랫폼 적용** | Codex 새 작업, Claude Code 새 세션·하위 에이전트, Antigravity 다음 턴·헤드리스 실행에 맞춘 얇은 어댑터 적용 |
| **변경 격리** | 모델명·가격·사용량 한도·임계값은 글로벌 룰에 고정하지 않고 버전이 있는 범위 정책으로 분리 |
