...
`enabledPlugins` 에도, 세션 런타임 스킬 목록에도 없다.

> ### ⚠️ 2026-08-06 정정 — 이 절의 핵심 논거 하나를 철회한다
>
> 원문은 **"약 2주간 부재를 아무도 인지하지 못했다 = 실사용 의존도가 낮았다는 실증"** 이라고
> 적었다. **사실이 아니다.** 소실 경위를 조사해 다음을 확인했다
> ([plugins.md §6](../../claude/docs/03_plugins.md)).
>
> | 원문 주장 | 실측 |
> |---|---|
> | 실사용 의존도가 낮음 | `usageCount` **105회** |
> | 약 2주간 부재 | 마지막 사용 **07-30 18:25**, 소실 **08-02**, 조사 **08-05** → **3일** |
> | 언제·왜 사라졌는지 미확인 | 플러그인 서브시스템 **전면 초기화**(설치본·설정·마켓플레이스 등록 전부). 행위자는 여전히 미특정 |
>
> **105회 사용은 낮은 의존도가 아니다.** 부재를 못 느낀 것이 아니라 부재 기간이 짧았을 뿐이다.
> 이 논거를 No-Go 근거에서 뺀다.
>
> **나머지 논거는 유효하다** — 14개 중 12개가 이미 커버되고, `writing-plans`↔`mia-strategic`,
> `test-driven-development`↔`test-writer` 트리거 충돌이 있으며, SessionStart 훅이 매 세션
> 코드를 실행한다. 다만 **결정의 확신도는 낮아졌다.** 재설치를 원하면 마켓플레이스 등록부터
> 다시 해야 한다(같은 문서 §6).
...
| Risk | SessionStart 훅 = 매 세션 코드 실행(공급망 노출면). ~~부재를 못 느낀 실사용 신호~~ **철회 — 위 정정 참조** |
