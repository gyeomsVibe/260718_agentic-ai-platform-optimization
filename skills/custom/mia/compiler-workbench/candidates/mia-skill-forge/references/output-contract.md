...
## User language inheritance

Default every generated candidate to Korean for user-facing descriptions, questions, examples, progress messages, error guidance, handoff reports, and Codex display metadata. Add this default explicitly to the candidate `SKILL.md`; do not assume that a user who does not know the setting exists will request it. Preserve exact identifiers, commands, paths, and maturity labels where translation would reduce technical accuracy. Switch to another language only when the user explicitly requests it.

## Beginner-safe inheritance

Assume the target user may not know which inputs, risks, or platform choices exist unless the source proves an expert-only audience. Add concise Korean guidance for the first required input, actions the Skill will not perform, and the next safe step. When a missing choice materially changes the result, explain why it matters, provide a safe default, and ask only one question. Never remove approval, evidence, or maturity boundaries to make the workflow feel easier.
