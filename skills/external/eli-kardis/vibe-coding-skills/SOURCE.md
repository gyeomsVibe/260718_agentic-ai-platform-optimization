...
`~/.agents/skills/`의 11개 `SKILL.md`가 스테이징본과 불일치했습니다.

> **2026-08-05 정정**: 이 절은 처음에 `~/.agents/skills/`를 "3대 도구 공용 경로"로 서술했습니다.
> 실측 결과 **Claude Code는 이 경로를 읽지 않습니다**(새 세션 스킬 목록에 40개 중 0개 노출).
> 아래 4번 항목의 "공용 경로" 표현도 같은 오해였습니다. 오염 자체와 원복 사실은 유효합니다.
