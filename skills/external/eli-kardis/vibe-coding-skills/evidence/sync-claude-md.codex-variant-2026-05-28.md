---
name: sync-Codex-md
description: This skill should be used when the user requests "sync", "동기화", "AGENTS.md 업데이트", "오늘 정리", "세션 정리", "하루 마무리", "/sync".
user-invocable: true
...
3개 소스(프로젝트 스캔 → 대화 컨텍스트 → git delta)를 종합하여 프로젝트 AGENTS.md를 생성/업데이트한다.

---

## 메모리 계층 이해

이 스킬은 **프로젝트 루트 `AGENTS.md`** (단일 파일)만 관리한다.

| 타입 | 위치 | 로딩 | 이 스킬 관리 |
|------|------|------|:----------:|
| Project memory | `./AGENTS.md` | 세션 시작 시 전체 로드 | **O** |
| Project local | `./Codex.local.md` | 세션 시작 시 전체 로드 (gitignore) | X |
| Project rules | `./.Codex/rules/*.md` | 세션 시작 시 전체 로드 | X |
| User memory | `~/.Codex/AGENTS.md` | 세션 시작 시 전체 로드 | X |
| Auto memory | `~/.Codex/projects/.../memory/` | MEMORY.md 200줄만 로드 | X |

**AGENTS.md에 넣을 것:** 빌드 명령어, 기술 스택, 프로젝트 구조, 코딩 컨벤션, 아키텍처 결정, 제약/주의사항, 워크플로우
**넣지 말 것:** 개인 선호 (`Codex.local.md` 사용), API 키/시크릿, 임시 메모, 미검증 추측
...
기존 AGENTS.md가 있을 때, 최근 변경사항만 **incremental 반영**한다.
...
> 참고: Codex `/init` 명령으로도 초기 AGENTS.md 생성 가능. 이 스킬은 `/init`보다 더 깊은 스캔과 대화 컨텍스트 반영을 제공한다.
...
- AGENTS.md에 **개인 선호** 기록 금지 (`Codex.local.md` 또는 Auto Memory 사용)
- **API 키, 시크릿, 인증 정보** 기록 금지
- **추측성 정보** 기록 금지 (스캔 또는 대화에서 확인된 사실만)
- 기존 AGENTS.md **구조 임의 변경** 금지
- **300줄 초과** 금지 (초과 시 압축하거나 `@import`로 분리)
- AGENTS.md를 **자동 커밋** 금지 (사용자 검토 후 직접 커밋)
...
### 스캔 결과와 기존 AGENTS.md 불일치
- **원인:** 패키지 업그레이드, 구조 변경 후 AGENTS.md 미반영
...
### Auto Memory와 중복
- **원인:** 같은 정보가 양쪽에 존재
- **해결:** 팀용 → AGENTS.md, 개인용 → MEMORY.md. 중복 시 MEMORY.md에서 제거 제안
...
| 안티패턴 | 문제 | 해결 |
|----------|------|------|
| 모든 것을 기록 | AGENTS.md 비대화 | 팀에 유용한 것만 선별 |
...
- `references/Codex-md-examples.md` - 프로젝트 규모별 AGENTS.md 예시 3종
